...
const excluded = ["(deps)", "(deps-dev)", "(auto)"];
...
module.exports = {
  ...
  ignores: [
    // Ignore deps updates as they might contain long strings
    message => excluded.some(exclusion => message.includes(exclusion))
  ],
  plugins: [
    {
      rules: {
